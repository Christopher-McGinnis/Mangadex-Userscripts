...
// })
function getUserValue(key: string ,defaultValue: unknown) {
    return new Promise((resolve ,reject) => {
        const jsonDefault = JSON.stringify(defaultValue)
        if (typeof GM === 'object' && typeof GM.getValue === 'function') {
            GM.getValue(key ,jsonDefault).then((value: string) => {
...
function getUserValues(keys: { [s: string]: unknown }) {
    const prommises: Promise<unknown[]>[] = []
...
function setUserValue(key: string ,value: unknown) {
    return new Promise((resolve ,reject) => {
...
function setUserValues(objs: { [s: string]: unknown }) {
    const prommises: Promise<unknown>[] = []
...
function createToolTip({ title ,text }: {title: string ;text: string}) {
    const tooltip_elm = htmlToElement(`<div>${title}<br><span>${text}</span></div>`) as HTMLElement
    const tooltip_text = tooltip_elm.children[1] as HTMLElement
