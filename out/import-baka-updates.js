...
;(() => {
  ...
  function EnableMdFeatureSubset(contentWindow ,contentDocument) {
    ...
    })).filter(({ btn ,selects }) => btn != null || selects.some(e => e.matches('.disabled.manga_rating_button'))).forEach(({ dropArrow ,btn ,selects }) => {
      let txtBtn = btn
      if (txtBtn == null) {
        txtBtn = dropArrow
      }
    ...
    Object.values(contentDocument.querySelectorAll('.btn-group')).map(e => ({
      ...
      ,dropArrow: e.querySelector('button.dropdown-toggle')
    })).filter(({ btn ,selects }) => btn != null || selects.some(e => e.matches('.disabled.manga_follow_button'))).forEach(({ dropArrow ,btn ,selects }) => {
      let txtBtn = btn
      if (txtBtn == null) {
        txtBtn = dropArrow
      }
    ...
    Object.values(contentDocument.querySelectorAll('.btn-group')).map(e => ({
      ...
      ,dropArrow: e.querySelector('button.dropdown-toggle')
    })).filter(({ btn ,selects }) => selects.some(e => e.matches('.manga_unfollow_button'))).forEach(({ dropArrow ,btn ,selects }) => {
      let txtBtn = btn
      if (txtBtn == null) {
        txtBtn = dropArrow
      }
  ...
  function fetchMdFollowsPages(page ,button ,container ,pages ,follows = []) {
    ...
    fetchMdFollowsPage(page ,button ,container ,pages).then(({ follows: thisPage ,pages }) => fetchMdFollowsPages(page + 1 ,button ,container ,pages).then(({ follows: nextPage ,pages }) => ({ follows: follows.concat(thisPage).concat(nextPage) }))).catch(e => ({
      follows
      ,pages
    }))
  }
  ...
  function createMdMatchInterface({ searchterm ,mangaList ,searchForm ,pages ,styles ,dom }) {
  ...
  function EasySelect(searchterm ,styles ,targetStatus) {
    return searchTitleMD(searchterm).then(({ mangaList ,searchForm ,pages ,dom }) => {
  ...
  function promptImportMd() {
    ...
    const importPromise = (targetStatus) => {
      ...
      if (list.length > 2) {
        ...
        let pageIdx = 1
        mdFilterList = loopRun(() => fetchMdFollowsPage(pageIdx).then(({ follows ,pages }) => {
